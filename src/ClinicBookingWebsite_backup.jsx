...
const faqs = [
  ["How do I register a slot?", "Select your provider, choose an active clinical slot, fill out the patient registry, and secure your position with the ₹100 commitment deposit via the Razorpay gateway."],
  ["Is the ₹100 fee mandatory?", "Yes. The ₹100 commitment deposit secures your queue position on the active doctor's daily roster, helping us minimize schedule overlap."],
...
export default function ClinicBookingWebsite() {
  ...
  // Re-established key form hook values
  ...
  const [bookings, setBookings] = useState(() => {
    ...
    return [
      { name: "Aman", doctor: "Dr Ajay Kumar", slot: "09:45 AM", status: "Confirmed", phone: "9835012345", age: "25", gender: "Male", reason: "Annual Health Assessment", date: todayLabel, token: 1 },
      { name: "Riya", doctor: "Dr Ajay Kumar", slot: "10:15 AM", status: "Pending", phone: "8210345678", age: "22", gender: "Female", reason: "Persistent Migraine Consultation", date: todayLabel, token: 2 },
      { name: "Kabir", doctor: "Dr Ajay Kumar", slot: "05:15 PM", status: "Completed", phone: "7004123456", age: "30", gender: "Male", reason: "Cardiovascular Review", date: todayLabel, token: 3 },
    ];
  });

  const [queueNo, setQueueNo] = useState(() => {
    const savedQueue = localStorage.getItem("clinicQueueNo");
    if (savedQueue !== null) {
      const num = Number(savedQueue);
      if (Number.isFinite(num) && num >= 1 && num !== 12) {
        return num;
      }
    }
    return bookings.length > 0 ? bookings.length + 1 : QUEUE_START;
  });
  ...
  useEffect(() => {
    ...
    if (now - savedResetAt >= QUEUE_RESET_MS) {
      localStorage.setItem("clinicQueueNo", String(QUEUE_START));
      localStorage.setItem("clinicQueueResetAt", String(now));
      setQueueNo(QUEUE_START);
      setBookingStatus("pending");
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("clinicQueueNo", String(queueNo));
  }, [queueNo]);
  ...
  const executeConfirmedBooking = () => {
    setPaymentLoading(true);
    const assignedToken = queueNo;
    setTimeout(() => {
      const nextBooking = {
        name: form.name.trim(),
        age: form.age,
      ...
      setLastConfirmedBooking(nextBooking);
      setBookingStatus("confirmed");
      setQueueNo((q) => {
        const next = q + 1;
        localStorage.setItem("clinicQueueNo", String(next));
        localStorage.setItem("clinicQueueResetAt", String(Date.now()));
        return next;
      });
      ...
      setToastMessage("Slot & Gateway Deposit Confirmed");
      setShowToast(true);
  ...
  return (
    <div className={`min-h-screen overflow-x-hidden ${theme} transition-colors duration-300 pb-28 md:pb-12 font-sans`}>
          ...
          <div className="flex items-center gap-1.5 ml-auto md:ml-0">
            ...
            <button 
              ...
              className={`p-2 rounded-lg border hover:scale-105 active:scale-95 transition-all ${
                darkMode ? "bg-zinc-900/50 border-zinc-800 text-zinc-300 hover:bg-zinc-800" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-100"
              }`}
          ...
          <div className="space-y-10 animate-fade-in">
              ...
              <div className="grid lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-7">
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-200 dark:border-zinc-900 grid grid-cols-2 gap-4">
                    {[["24/7", "Clinical Registry"], ["UPI", "Razorpay Secured"]].map(([a, b]) => (
                      <div key={a}>
                  ...
                  <div className={`rounded-2xl p-5 border ${darkMode ? "bg-zinc-950/80 border-zinc-900" : "bg-zinc-50 border-slate-200"}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className={`rounded-xl p-4 border transition-all duration-300 hover:scale-105 ${darkMode ? "bg-zinc-900/40 border-zinc-900/80" : "bg-white border-slate-200"}`}>
                        ...
                        <div className="text-lg font-bold mt-2 text-zinc-900 dark:text-zinc-100 font-mono">#{queueNo}</div>
                      ...
                      <div className={`rounded-xl p-4 border transition-all duration-300 hover:scale-105 ${darkMode ? "bg-zinc-900/40 border-zinc-900/80" : "bg-white border-slate-200"}`}>
                        ...
                        <div className="text-lg font-bold mt-2 text-zinc-900 dark:text-zinc-100 font-mono">₹100</div>
                        <p className={`text-[10px] mt-1 ${muted}`}>Razorpay Secured</p>
                      </div>
                      <div className={`rounded-xl p-4 border transition-all duration-300 hover:scale-105 ${darkMode ? "bg-zinc-900/40 border-zinc-900/80" : "bg-white border-slate-200"}`}>
            ...
            <section className="grid lg:grid-cols-2 gap-6">
              <div className={`rounded-[1.5rem] p-6 border ${darkMode ? "bg-zinc-900/40 border-zinc-800" : "bg-white border-slate-200"}`}>
                <h2 className="text-lg font-bold mb-4 tracking-tight">Clinical Infrastructure</h2>
              ...
              <div className={`rounded-[1.5rem] p-6 border ${darkMode ? "bg-zinc-900/40 border-zinc-800" : "bg-white border-slate-200"}`}>
                <h2 className="text-lg font-bold mb-4 tracking-tight">Verified Patient Feedback</h2>
            ...
            <section className={`rounded-[1.5rem] p-6 border ${darkMode ? "bg-zinc-900/50 border-zinc-800" : "bg-teal-50/50 border-teal-100"}`}>
              <div className="grid lg:grid-cols-2 gap-5 items-center">
                <div>
                  ...
                  <p className={`mt-2 text-xs md:text-sm leading-relaxed ${muted}`}>Active roster allocations are updated immediately. Secure your token by processing the commitment deposit via Razorpay.</p>
                </div>
            ...
            <section className={`rounded-[1.5rem] p-6 border ${darkMode ? "bg-zinc-900/40 border-zinc-800" : "bg-white border-slate-200"}`}>
              <h2 className="text-lg font-bold mb-4 tracking-tight">Facility Environment</h2>
        ...
        {page === "booking" && (
          <section className="grid lg:grid-cols-12 gap-6 animate-fade-in">
            ...
            <div className={`rounded-[1.5rem] p-6 border lg:col-span-7 ${darkMode ? "bg-zinc-900/30 border-zinc-800" : "bg-white border-slate-200"}`}>
              <h2 className="text-xl font-bold mb-4 tracking-tight">Patient Registration</h2>
              
              <div className="space-y-5">
                <div>
                  <label className="text-[10px] tracking-wider uppercase block mb-1.5 opacity-80 text-zinc-700 dark:text-zinc-300 font-bold">Consulting Doctor</label>
                  <select 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] tracking-wider uppercase block mb-1.5 opacity-80 text-zinc-700 dark:text-zinc-300 font-bold">Patient Full Name <span className="text-rose-500">*</span></label>
                    <input
                      value={form.name}
                  ...
                  <div>
                    <label className="text-[10px] tracking-wider uppercase block mb-1.5 opacity-80 text-zinc-700 dark:text-zinc-300 font-bold">Patient Age (Years) <span className="text-rose-500">*</span></label>
                    <input
                      value={form.age}
              ...
              <div className="mt-6 space-y-3">
                ...
                >
                  Confirm Slot & Secure ₹100 Deposit
                </button>
            ...
            <div className={`rounded-[1.5rem] p-6 border lg:col-span-5 ${darkMode ? "bg-zinc-900/30 border-zinc-800" : "bg-white border-slate-200"} flex flex-col justify-between`}>
              <div>
                <h2 className="text-xl font-bold mb-4 tracking-tight">Appointment Receipt</h2>
                <div className={`rounded-2xl p-5 border ${darkMode ? "bg-zinc-950/80 border-zinc-900" : "bg-slate-50 border-slate-200"}`}>
                  ...
                  <div className={`mt-5 p-4 rounded-xl border text-[11px] leading-relaxed ${darkMode ? "bg-zinc-900/50 border-zinc-800 text-zinc-400" : "bg-white border-slate-200 text-slate-600"}`}>
                    Sanjivani Clinic runs an automated patient intake. Once the commitment deposit is cleared via Razorpay, your slot and SMS update tracker are registered.
                  </div>
                </div>
              </div>
        ...
        {page === "admin-login" && (
          <section className="max-w-md mx-auto animate-fade-in">
            <div className={`rounded-[1.5rem] p-6 border ${darkMode ? "bg-zinc-900/30 border-zinc-800" : "bg-white border-slate-200"}`}>
              ...
              <h2 className="text-xl font-bold mb-1 tracking-tight">Staff Portal Access</h2>
        ...
        {page === "admin" && adminUnlocked && (
          <section className="space-y-6 animate-fade-in">
            <div className={`rounded-[1.5rem] p-6 border ${darkMode ? "bg-zinc-900/30 border-zinc-800" : "bg-white border-slate-200"} flex flex-col md:flex-row gap-4 items-start md:items-center justify-between`}>
              <div>
                <h2 className="text-xl font-bold mb-1 tracking-tight">Staff Workspace Console</h2>
                      ...
                      filteredBookings.map((booking, index) => (
                        <tr key={`${booking.name}-${index}`} className={`border-b hover:bg-teal-500/[0.02] transition-all ${darkMode ? "border-zinc-900/40" : "border-slate-100"}`}>
                          <td className="py-4 px-2">
                            ...
                            <div className="text-[10px] tracking-wide uppercase text-zinc-400 dark:text-zinc-500 mt-1">
      ...
      {/* RAZORPAY ₹100 BOOKING DEPOSIT SECURE MODAL */}
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm shadow-xl" onClick={() => setShowPaymentModal(false)} />
          
          <div className={`relative max-w-sm w-full rounded-2xl p-6 ${card} z-10 border border-slate-200 dark:border-zinc-800 animate-scale-up`}>
            <button 
              onClick={() => setShowPaymentModal(false)} 
              className="absolute top-4 right-4 p-2 rounded-lg hover:bg-teal-500/10 transition-colors"
            >
              <X size={14} />
            </button>

            <div className="text-center">
              <div className="h-10 w-10 bg-teal-500/10 text-teal-600 dark:text-teal-400 mx-auto rounded-xl flex items-center justify-center mb-3 border border-teal-500/20 shadow-sm animate-pulse">
                <ShieldCheck size={20} />
              </div>
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-1 tracking-tight">Razorpay Secure Network</h3>
              <p className="text-zinc-500 text-[10px] uppercase tracking-wider font-bold">Standard Deposit Processing</p>

              <div className={`my-5 p-4 rounded-xl border ${darkMode ? "bg-zinc-950/80 border-zinc-900" : "bg-slate-50 border-slate-150"}`}>
                <div className="text-[9px] text-zinc-500 uppercase font-bold mb-1">Intake Registry Fee</div>
                <div className="text-2xl font-black text-teal-600 dark:text-teal-400 tracking-wider font-mono">₹100.00</div>
                <p className="text-[10px] text-zinc-600 dark:text-zinc-400 mt-2 leading-relaxed">
                  Roster segment held for <strong className="text-zinc-800 dark:text-zinc-200">{form.name}</strong> at <strong className="text-zinc-800 dark:text-zinc-200">{selectedSlot}</strong>. 
                  Your appointment slot is only secured in the queue once payment is authorized through the Razorpay interface.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                >
                  {paymentLoading ? (
                    <>
                      <Loader2 size={12} className="animate-spin" /> Verifying Gateway...
                    </>
                  ) : (
                ...
                <div className="flex items-center justify-center gap-1.5 text-[9px] text-zinc-500 font-bold uppercase tracking-widest">
                  <ShieldCheck size={10} className="text-emerald-500" /> AES-256 secure checkout
                </div>
              </div>
            </div>
          </div>
